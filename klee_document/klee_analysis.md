...
则符号执行的表示如下：
  ...
  **value**: (10,10)  **path**：1-3-5-6  **path_condition** !(a<10)&&!(b<10) 
  ...
  在执行每条指令时，对于非分支指令，可通过函数的复合操作来更新路径函数。对于分支指令，由于要执行一条新路径，所以需要创建新的进程，并将原路径条件与每个分支目标满足的分支条件执行合取逻辑操作，形成新的路径条件。当某符号执行进程结束时，可调用约束求解器求解路径条件以获取可导致程序执行该路径的具体输入值的集合。当所有符号执行的进程都结束了，则整个符号执行过程结束。	
